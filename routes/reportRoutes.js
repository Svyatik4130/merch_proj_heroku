...
router.post('/add', auth, async (req, res) => {
    try {
        ...
        // console.log(req.body.imagesNames)
        // res.json(req.body)
        // console.log(req.files)
        const { reportValue, locTitle, userName, date, imagesNames } = req.body
...
router.get("/getallreports", auth, async (req, res) => {
    // await locationReport.deleteMany({})
...
router.get("/getuserreports", auth, async (req, res) => {
    try {
        const allReports = await locationReport.find({reporterID: req.user})
        res.json(allReports)
    } catch (error) {
        res.status(500).json(error.message)
    }
